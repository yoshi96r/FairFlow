...
def auto_assign(session: Session, max_open_per_emp: int = 5) -> int:
    ...
    emp_load: Dict[int, int] = {e.id: session.exec(
        select(WorkOrder).where(WorkOrder.assigned_employee_id == e.id, WorkOrder.status != "Done")
    ).count() for e in employees}
    ...
    for wo in sorted(unassigned, key=lambda w: (w.priority, w.due_date or date.max)):
        ...
        scored = []
        for e in employees:
            if emp_load[e.id] >= max_open_per_emp:
                continue
